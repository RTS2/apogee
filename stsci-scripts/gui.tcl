...
global TEMPS AVGTEMPS STATUS
  if { $STATUS(tempgraph) }  {
   ...
   if { $TEMPS == "" } {
       ...
       set TEMPS [lrange "$t $TEMPS" 0 1]
   }
   set i 0
   set temp 0
   while { $i < 2 } {set temp [expr $temp+[lindex $TEMPS $i]] ; incr i 1}
   set AVGTEMPS [expr $temp/2.0]
   plottemp
   after 3000 monitortemp
  }
}
#
# PRM 15Jun2002, the following was the original code; we speeded things up a bit
# and made the frequency response higher with the code above. Should return
# to the original code after debugging erratic Cooler control on startup.
#
#global TEMPS AVGTEMPS STATUS
#  if { $STATUS(tempgraph) }  {
#   set t [lindex [get_temp $id] 0]
...
# the follwing two should be set by .apgui.tcl
#if { ![file exists $env(HOME)/.apgui.tcl] } {
#   set SCOPE(driftdelay) 80000
#   set SCOPE(driftrows) 412
#   }
...
### PRM, 15Jun2002, make it confirm Cooler actions
###.mbar.temp.m add command -label "Cooler on" -command "setpoint on"
###.mbar.temp.m add command -label "Cooler off" -command "setpoint off"
.mbar.temp.m add command -label "Cooler on" -command  {set ok [confirmaction "Cool to $SETPOINTS"] ; if {$ok} {setpoint on}}
.mbar.temp.m add command -label "Cooler off" -command  {set ok [confirmaction "Turn Cooler off (ramp to ambient is better)"] ; if {$ok} {setpoint off}}
...
button .drift.open -text "Shutter Open(1)" -command "set SCOPE(shutter) 1"
place .drift.open -x 10 -y 10
button .drift.closed -text "Shutter Closed(0)" -command "set SCOPE(shutter) 0"
place .drift.closed -x 130 -y 10
entry .drift.vshut -bg white -fg black -width 1 -textvariable SCOPE(shutter)
place .drift.vshut -x 290 -y 10
...
label .drift.mmsec -text "Microsecond delay per row =  "
place .drift.mmsec -x 10 -y 230
label .drift.vmsec -textvariable SCOPE(driftdcalc)
place .drift.vmsec -x 290 -y 230
#button .drift.go -text "Start drift scan exposure" -width 46 -height 3 -bg gray -command "driftscan 0 0 1 $SCOPE(shutter)"
button .drift.go -text "Start drift scan exposure" -width 46 -height 3 -bg gray -command "driftscan 0 0 1 0"
...
#
#---------------------------------------------------------------------------
# all these driftscan procedures moved into tdi.tcl by PRM Jun 22, 2002
#
