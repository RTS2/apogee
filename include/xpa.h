...
#define XPA_PATCH_LEVEL 5
#define XPA_VERSION "2.1.5"
...
typedef struct xparec{
  ...
  int message;		/* flag if an acknowledgement message was sent */
...
int XPANSKeepAlive _PRx((XPA xpa));
...
int Xpa_Init _PRx((void *vinterp));
/* xpaio.c */
void XPAAlarmStart _PRx((int seconds));
void XPAAlarmEnd _PRx((void));
int XPAAlarmCheck _PRx((void));
int rdl _PRx((int fd, char *buf, int len));
int XPAGets _PRx((int fd, char *buf, int len, int timeout));
int XPAPuts _PRx((int fd, char *buf, int timeout));
int XPAGetBuf _PRx((int fd, char **buf, int *len, int timeout));
int XPAPutBuf _PRx((int fd, char *buf, int len, int timeout));
int XPASetBuf _PRx((XPA xpa, char *buf, int len, int xcopy));
int XPARdl _PRx((int fd, char *buf, int len, int timeout));
void XPADefaultSigaction _PRx((void));
