...
void main()
{
	...
	printf( "Apogee Alta Streaming TDI Sample Applet\n" );
	...
	long ImgYSize = 1;		// Since SequenceBulkDownload = false
	...
	sprintf( szFilename, "StreamingTdiData.raw" );
	...
	{
		while ( AltaCamera->TDICounter != i )
		{
			// printf( "Waiting for next TDI row\n" );
		}

		// Get the line data from the camera
		printf( "Retrieving line %d data from camera...\n", i );
