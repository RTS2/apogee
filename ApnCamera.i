/* Copy  from ApnCamera.h and replace all xxxMode typedefs with
  int or unsigned short so swig can grok them
 */
...
public:
	...
	void		SetNetworkTransferMode( int TransferMode );
	...
	int				read_CameraMode();
	void				write_CameraMode( int CameraMode );

	int				read_DataBits();
	void				write_DataBits( int BitResolution );

	int				read_ImagingStatus();

	int				read_LedMode();
	void				write_LedMode( int LedMode );
	int				read_LedState( unsigned short LedId );
	void				write_LedState( unsigned short LedId, int LedState );

	bool				read_CoolerEnable();
	void				write_CoolerEnable( bool CoolerEnable );
	int				read_CoolerStatus();
	...
	int				read_FanMode();
	void				write_FanMode( int FanMode );
	...
	int				read_NetworkTransferMode();
	void				write_NetworkTransferMode( int TransferMode );
...
//	bool				read_FilterPositioningDone();
...
/* was private */
	...
	int			m_pvtCoolerStatus;
	int			m_pvtImagingStatus;
	int			m_pvtPrevImagingStatus;
